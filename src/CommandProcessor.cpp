...
bool cCommandProcessor::gpio (String &payloadStr, String &ControllerName, gpio_num_t pin, String &Response)
{
    ...
    do  // once
    {
        ...
        {
            Log.infoln (
                (String (F ("-> ")) + ControllerName + F (" Controller: Read GPIO Pin-") + String (pin) + F (", Value= ") +
        ...
        {
            Log.errorln (
                (String (F ("-> ")) + ControllerName + F (" Controller: Invalid GPIO Payload (") + payloadStr + F (
                "), Ignored.")).c_str ());
            response = false;
            break;
        }
        ...
        Log.infoln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Setting GPIO Pin-") + String (pin) + F (" to ") +
...
bool cCommandProcessor::info (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    {
        Log.errorln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Invalid INFO Payload (") + payloadStr + F (
            "), Ignored.")).c_str ());
        response = false;
    }
    DEBUG_END;
...
bool cCommandProcessor::log (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    do  // once
    {
        ...
        Log.errorln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Invalid LOG Payload (") + payloadStr + F (
            "), Ignored.")).c_str ());
        response = false;
    } while (false);
...
bool cCommandProcessor::mute (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    do  // once
    {
        ...
        Log.errorln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Invalid MUTE Payload (") + payloadStr + F (
            "), Ignored.")).c_str ());
        response = false;
    } while (false);
...
bool cCommandProcessor::piCode (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    do  // once
    {
        ...
        {
            Log.errorln (
                (String (F ("-> ")) + ControllerName + F (" Controller: Invalid RDS PI Code Value (") + payloadStr +
                F ("), Ignored.")).c_str ());
            response = false;
            break;
        }
        ...
        {
            Log.verboseln (
                (String (F ("-> ")) + ControllerName + F (" Controller: PI Code Unchanged (0x") +
        ...
        // displaySaveWarning();
        Log.verboseln (
            (String (F ("-> ")) + ControllerName + F (" Controller: PI Code Set to (0x") + String (tempPiCode, HEX) + F (
            ").")).c_str ());
    } while (false);
...
bool cCommandProcessor::ptyCode (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    do  // once
    {
        ...
        {
            Log.errorln (
                (String (F ("-> ")) + ControllerName + F (" Controller: Invalid RDS PTY Code Value (") + payloadStr +
                F ("), Ignored.")).c_str ());
            response = false;
            break;
        }
        ...
        {
            Log.verboseln (
                (String (F ("-> ")) + ControllerName + F (" Controller: PTY Code Unchanged (0x") +
        ...
        // displaySaveWarning();
        Log.verboseln (
            (String (F ("-> ")) + ControllerName + F (" Controller: PTY Code Set to (0x") + String (tempPtyCode, HEX) + F (
            ").")).c_str ());
    } while (false);
...
bool cCommandProcessor::rdsTimePeriod (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    do  // once
    {
        ...
        {
            Log.verboseln (
                (String (F ("-> ")) + ControllerName + F (" Controller: RDS Time Period Value out-of-range, set to ") +
                String (rtTime) + " secs.").c_str ());
        }
        else
        {
            Log.verboseln (
                (String (F ("-> ")) + ControllerName + F (" Controller: RDS Time Period Value set to ") + String (rtTime) +
                " secs.").c_str ());
        }
    } while (false);
...
bool cCommandProcessor::reboot (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    {
        Log.errorln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Invalid REBOOT Payload (") + payloadStr + F (
            "), Ignored.")).c_str ());
        response = false;
    }
    DEBUG_END;
...
bool cCommandProcessor::rfCarrier (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    do  // once
    {
        ...
        Log.errorln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Invalid RF Carrier Payload (") + payloadStr + F (
            "), Ignored.")).c_str ());
        response = false;
    } while (false);
...
bool cCommandProcessor::start (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    {
        Log.errorln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Invalid START Payload (") + payloadStr + F (
            "), Ignored.")).c_str ());
        response = false;
    }
    DEBUG_END;
...
bool cCommandProcessor::stop (String &payloadStr, String &ControllerName, String &Response)
{
    ...
    {
        Log.errorln (
            (String (F ("-> ")) + ControllerName + F (" Controller: Invalid STOP Payload (") + payloadStr + F (
            "), Ignored.")).c_str ());
    }
    DEBUG_END;
